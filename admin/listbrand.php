...
<?php 
	...
	if(isset($_REQUEST['search']))
	{
		$name=$_REQUEST['item'];
		if(empty($name))
			header("location:index.php?view=listbrand.php");
		$sql = "select * from brand where brand_name like '$name%' or brand_desc like '$name%'";
	...
	{		
		...
		$delsql="delete from brand where brand_id in (".$d.") ";
...
<form method="post" action="index.php?view=listbrand.php">
...
<table align="center" width="70%" height="50%" class="table">
	<th colspan=5 class="th">LIST OF BRANDS</th>
	<tr>
		<td class="tr">ID</td>
		<td class="tr">Name</td>
	...
	<tr>
		...
		<td><?=$row[1]?></td>
		<td><?=$row[2]?></td>
		<td><a style="width:40%" href="index.php?view=addbrand.php&brand_id=<?=$row[0]?>">Edit</a></td>
		<td>
